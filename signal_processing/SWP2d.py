...
class Signal():
    ''' The Signal Class is intended to be used by the SWP2d Class as a way to store 1d - 4d numpy arrays and key attributes in a consistent way. These array shapes should be in the form (N x D x H x W), where:
        ...
        Attributes:
            self.array: 1d, 2d, 3d, or 4d numpy array
            self.num: the axis 0 dimension of the 4d array, else None
            self.depth: the axis 0, 1 dimension of the 3d, 4d array respectively, else None
            self.height: the axis 0, 1, 2 dimension of the 2d, 3d, 4d array respectively, else None
            self.width: the axis 0, 1, 2, 3 dimension of the 1d, 2d, 3d, 4d array respectively, else None
    '''

    def __init__(self, array):
        ''' Signal object is initialized by first converting the input x into a numpy array, then getting the number of dimensions. This is then used to assign the correct values to each of the dimension attributes.

            Arguments:
                array: Array or array-like object that is able to be converted into Numpy Array with between 1 - 4 dimensions
        '''

        # assign x as numpy array and get number of dimensions
        self.array = np.asarray(array)
        n = array.ndim

        assert 1 <= n <= 4, f"Invalid Number of Dimensions ({n}) for Signal Object. Must be between 1 and 4"

        # function to get the size of a dimension along a given axis
        get_dim = lambda x, axis : array.shape[axis] if axis >= 0 else None

        # Create dimensions list
        dims = []

        # Iterate down from 4 to 1
        for i in range(4, 0, -1):

            # Create list of dimension size along each axis
            axis = n-i
            dims.append(get_dim(array, axis))

        # Assign dimension size to each variable
