...
def convblock(in_channels, out_channels, kernel_size, stride):
    ...
    # Padding for 'same' conv output size as input size
    padding = int(np.floor((kernel_size - 1) / 2))
    padding_mode = 'reflect'
    args = (in_channels, out_channels, kernel_size, stride, padding, padding_mode)
...
if __name__ == '__main__':
    ...
    net = UNet(input_channels=c, output_channels=c, encoder_channels=encoder_channels, kernel_sizes=[3, 3], kernel_strides=[1, 1], maxpool_size=k, maxpool_stride=s)
