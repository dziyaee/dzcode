...
# Inputs
images_shape = (1, 1, 500, 500)
kernels_shape = (2, 1, 3, 3)
padding = (1, 1)
stride = (1, 1)
repeat = 2 # min 2 reps
number = 2
...
# Timing
# Minimum two repetitions (first repetition is slower than normal)
repeat = max(repeat, 2)
...
def timeit_tests(setups, stmts, repeat, number):
    ...
    for stmt, setup in zip(stmts, setups):
        ...
        times = np.asarray(times)[1:]  # ignore first rep for timings
        avg_time = float((times / (repeat - 1)) / number)
